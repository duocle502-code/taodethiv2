...
export default function GeneratorView({ data, onSaveData }: GeneratorViewProps) {
  ...
  const handleGenerate = async () => {
    ...
    const mathRules = [
      ...
      '',
      '**HÌNH HỌC KHÔNG GIAN (3D) — QUY TẮC NÉT VẼ (CỰC KỲ QUAN TRỌNG):**',
      '- NÉT LIỀN (stroke, KHÔNG có stroke-dasharray): chỉ dùng cho các cạnh NHÌN THẤY (ở phía trước, phía trên).',
      '- NÉT ĐỨT (stroke-dasharray="8,5"): chỉ dùng cho các cạnh BỊ KHUẤT (ở phía sau, phía dưới, bị che bởi mặt phẳng).',
      '- KHÔNG ĐƯỢC vẽ tất cả cạnh bằng nét liền. KHÔNG ĐƯỢC vẽ tất cả cạnh bằng nét đứt.',
      '- Quy tắc phối cảnh xiên (oblique projection):',
      '  + Mặt trước vẽ đúng hình dạng thật (vuông, chữ nhật, tam giác...).',
      '  + Các cạnh đi vào sâu vẽ xiên 45° về phía trái-trên, co ngắn ~60% chiều dài thật.',
      '  + Các cạnh ở mặt sau bị khuất → NÉT ĐỨT.',
      '- Ví dụ hình hộp chữ nhật ABCD.A\'B\'C\'D\':',
      '  + Cạnh nhìn thấy (NÉT LIỀN): AB, BC, CD, DA (mặt trước), AA\', BB\', CC\' (cạnh bên nhìn thấy), A\'B\', B\'C\' (mặt trên/bên nhìn thấy).',
      '  + Cạnh khuất (NÉT ĐỨT): DD\', C\'D\', A\'D\' (bị che).',
      '- Ví dụ hình chóp S.ABCD:',
      '  + NÉT LIỀN: SA, SB, SC, SD, AB, BC (trước).',
      '  + NÉT ĐỨT: AD, CD (cạnh đáy bị khuất).',
      '- Màu nét liền: stroke="#1D4ED8" stroke-width="2.5".',
      '- Màu nét đứt: stroke="#1D4ED8" stroke-width="1.5" stroke-dasharray="8,5".',
      '- Đường phụ (đường cao, khoảng cách...): stroke="#DC2626" stroke-dasharray="6,4" stroke-width="1.5".',
      '',
